...
- **文章点评**（每篇文章底部"有帮助 / 没帮助"按钮，用 KVdb 免费存储计数）
...
每篇文章底部有"有帮助 / 没帮助"两个按钮，点击即投票，计数用 [KVdb](https://kvdb.io) 免费存储。

> **注意**：首次启用需要验证一下 KVdb 账号邮箱（发到 `config.json` 里 `feedback` 对应的邮箱，点邮件里的确认链接一次即可），否则投票只会存在访问者本地、不会累计到服务端。配置在 `config.json` 的 `feedback` 段。
